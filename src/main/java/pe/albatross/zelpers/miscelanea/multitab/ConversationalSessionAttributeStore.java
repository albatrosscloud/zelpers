package pe.albatross.zelpers.miscelanea.multitab;
 ...
 *
 *
 * Note 2: This is a modification of the code in the URL because I don't want to
 * use a tag lib. Therefore - my implementation creates a conversation map on
 * the session - to allow each conversation to have its own store without adding
 * the cid on the object name. In this way - the cid and the session attributes
 * are kept separated.
 *
...
public class ConversationalSessionAttributeStore implements SessionAttributeStore, InitializingBean {
    ...
    private static final Logger logger = getLogger(ConversationalSessionAttributeStore.class);
     ...
     * @param attributeName - the name of the attribute (from
     * @SessionAttributes)
